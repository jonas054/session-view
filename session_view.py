...
_FAVICON_SVG = _load_asset_text("static/icons/copilot.svg")
...
def _render_edit_diff(old: str, new: str) -> str:
    ...
    for line in diff:
        if line.startswith("+++") or line.startswith("---"):
            ...
            rows.append(line)
    return _render_unified_diff_text("\n".join(line.rstrip("\n") for line in rows))
...
def render_html(overview: dict, turns: list, events: list, source_path: str, a11y: bool = False, story_text: str | None = None) -> str:
    ...
    # Story tab
    if story_text:
        def _story_block(block: str) -> str:
            t = block.strip()
            if t.startswith("Story generated by"):
...
def _transform_file_path(m: re.Match) -> str:
    """Convert a file path match to a TTS-friendly description."""
    path = m.group(0)
    ...
    filename_tts = filename.replace('.', ' dot ')
    return f'{filename_tts} in {parent}' if parent else filename_tts
...
def _tts_transform(text: str) -> str:
    ...
    # "codebase" → "code base" (preserve original case)
    text = re.sub(r'(?i)\bcodebase\b', lambda m: m.group(0)[:4] + ' ' + m.group(0)[4:], text)
    # "todo/todos" → "to-do/to-dos" (always lowercase)
    text = re.sub(r'\btodos?\b', lambda m: 'to-dos' if m.group(0).lower() == 'todos' else 'to-do', text, flags=re.IGNORECASE)
    # File paths: "filename.ext in parent" with dots spelled out
    text = re.sub(r'(?<![:\w])/[a-zA-Z_\d][\w.-]*(?:/[\w.-]+)*', _transform_file_path, text)
    text = re.sub(r'\b[a-zA-Z_][\w.-]*/[\w.-]+(?:/[\w.-]+)*\b', _transform_file_path, text)
    # camelCase/PascalCase: insert hyphens so TTS reads them as separate words
    # (HTTPClient → HTTP-Client, getUserById → get-User-By-Id).
...
def generate_story(jsonl_path: str, force: bool = False, language: str = None) -> str | None:
    ...
    _parse_json_events(jsonl_path, user_name=user_name)  # ensure events.txt is up to date
    ...
    prompt = (
        ...
        "minimum. Divide the story into small chapters, "
        "and make it an interesting read for a developer using plot twists and cliffhangers "
        "as story-telling devices. Start with the heading text in "
        f"'Story generated by {model}' (heading in English). Output the story directly as plain "
        "text in your response — do not write it to any file and do not use any tools.\n"
        f"The user's name is {user_name!r}. Use it naturally when referring to the user.\n"
        "\n"
        "<session_events>\n"
        f"{events_txt}"
        "\n\n</session_events>\n"
