...
.reasoning-step[open] > .tool-summary {
...
.reasoning-body .md-h6 {
  color: #4c1d95;
  border-color: #ddd6fe;
}
.reasoning-body .md-blockquote {
  background: #f5f3ff;
  border-left-color: #c4b5fd;
  color: #5b21b6;
}
.reasoning-body code.md-code {
  background: white;
  border-color: #ddd6fe;
  color: #7c3aed;
}
.reasoning-body .md-table th,
.reasoning-body .md-table td {
  border-color: #ddd6fe;
}
.reasoning-body .md-table th {
  background: #f5f3ff;
...
.snippet-row { cursor: pointer; }
.snippet-row:hover td { background: #fef3c7; }
/* Add a white gap above a group header when it immediately follows a snippet row */
tr.snippet-row + tr.group-header td {
...
function escapeRegExp(s) {
  // Avoid complicated backslash escaping in the source by constructing the
  // escaped characters programmatically.
  const specials = "*+?^${}()|[]\\/."; // characters that should be escaped
  let out = "";
  for (let i = 0; i < s.length; i++) {
    const ch = s[i];
    // Use String.fromCharCode(92) ("\\") to produce a literal backslash
    // without embedding fragile escaped sequences in the source.
    out += (specials.indexOf(ch) >= 0) ? String.fromCharCode(92) + ch : ch;
  }
  return out;
}
...
function render() {
  ...
  filtered.forEach(item => {
    ...
    if (!useGroups || !collapsed.has(gk)) {
      ...
      // If a query is present and the prompt did not match, attempt to generate a highlighted snippet
      if (q && !promptMatched) {
...
def build_overview_html(sessions: list) -> str:
    ...
    js = OVERVIEW_JS.replace("__DATA__", data_json)
    count = len(sessions)
