...
def test_extract_intents_prefers_embedded_requests_and_deduplicates_execution_starts():
    events = [
        {
            ...
            "data": {
                "toolRequests": [
                    {
                        "name": "report_intent",
                        "toolCallId": "tc-primary",
                        "arguments": {"intent": "Embedded intent"},
                    },
                ],
            },
        },
        {
            ...
            "data": {
                ...
                "arguments": {"intent": "Execution intent"},
            },
        },
        {
            "type": "tool.execution_start",
            "data": {
                ...
                "toolName": "report_intent",
                "arguments": {"intent": "Fallback intent"},
            },
        },
    ]

    assert sv._extract_intents(events) == ["Embedded intent", "Fallback intent"]
    assert sv.build_overview(events)["intents"] == ["Embedded intent", "Fallback intent"]
...
def test_read_session_collects_search_text(rich_session_dir):
...
def test_read_session_indexes_embedded_intents_without_execution_starts(tmp_path):
    session_dir = tmp_path / "embedded-intent"
    session_dir.mkdir()
    events = [
        {
            "type": "assistant.message",
    ...
    assert info["search_fields"]["intents"] == "Embedded intent"
